  ...
  methods: {
	...
	handleUpload(param){
		...
		let form = new FormData();
		    ...
		    form.append('img',param.file);//传文件
		    //form.append('projectId',self.projectId);//传projectId
			 axios.post('/api/common/uploadImg', form)
			    .then(res=>{
					//res = res.data
					//param.Success()
			       ...
			       if (res.code === 1) {
					...
					console.log(param)
			        _this.emitInput(getArrByKey(fileList, 'url'))
			      } else {
			        for (let i = 0; i < fileList.length; i++) {
			          if (fileList[i]['uid'] === file['uid']) {
			            fileList.splice(i, 1)
